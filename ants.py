...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if self.ant is insect and insect == QueenAnt.true_queen_ant:
                self.ant = insect
            elif self.ant.is_container and self.ant.contained_ant == QueenAnt.true_queen_ant and insect == QueenAnt.true_queen_ant:
                self.ant.contained_ant = insect
                insect.place = self
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant:
            return not self.place.ant.blocks_path == False
        else:
            return False
        # END Problem 7
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared_bee and self.current_direction == 1:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        check_location = self.place
        range_of_attack = 0
        while range_of_attack < self.min_range:
            check_location = check_location.entrance
            range_of_attack += 1
        while check_location is not hive and range_of_attack <= self.max_range:
            if check_location.bees:
                return random_or_none(check_location.bees)
            else:
                check_location = check_location.entrance
                range_of_attack += 1
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            given_bee = random_or_none(self.place.bees)
            self.eat_bee(given_bee)
        # END Problem 6
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does nothing each turn, but adds protection through its armor."""
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.armor = armor
        if QueenAnt.true_queen_ant == None:
            QueenAnt.true_queen_ant = self
        else:
            QueenAnt.imposter_ant = self
        # END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self == QueenAnt.imposter_ant:
            QueenAnt.reduce_armor(self, self.armor)
        else:
            self.throw_at(self.nearest_bee(colony.hive))
            current = self.place.exit
            while current is not None:
                if current.ant not in QueenAnt.buffed_ants and current.ant is not None:
                    current.ant.damage *= 2
                    QueenAnt.buffed_ants.extend([current.ant])
                    if current.ant.is_container and current.ant.contained_ant is not None and current.ant.contained_ant not in QueenAnt.buffed_ants:
                        current.ant.contained_ant.damage *= 2
                        QueenAnt.buffed_ants.extend([current.ant.contained_ant])
                    elif current.ant.is_container and current.ant.contained_ant is not None and current.ant not in QueenAnt.buffed_ants:
                        current.ant.damage *= 2
                        QueenAnt.buffed_ants.extend([current.ant])
                elif current.ant is not None and current.ant.is_container and current.ant in QueenAnt.buffed_ants:
                    if current.ant.contained_ant is not None and current.ant.contained_ant not in QueenAnt.buffed_ants:
                        current.ant.contained_ant.damage *= 2
                        QueenAnt.buffed_ants.extend([current.ant.contained_ant])
    ...
    def reduce_armor(self, amount):
        ...
        if self == QueenAnt.true_queen_ant and self.armor <= 0:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        nonlocal duration
        if duration > 0:
            effect(bee_action, bee)(colony)
            duration -= 1
        else:
            bee_action(colony)
            bee.action = bee_action
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target.scared_bee == False:
